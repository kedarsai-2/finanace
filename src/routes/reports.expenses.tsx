...
export const Route = createFileRoute("/reports/expenses")({
  head: () => ({ meta: [{ title: "Expense Report — Ledgerly" }] }),
  ...
  return (
      ...
      filters={
        <>
          ...
          {(from || to || category !== "all" || accountId !== "all") && (
            <Button variant="ghost" size="sm" onClick={() => { setFrom(""); setTo(""); setCategory("all"); setAccountId("all"); }}>
              Reset
            </Button>
          )}
        </>
    ...
    >
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        {rows.length === 0 ? (
          <div className="px-6 py-16 text-center text-sm text-muted-foreground">No expenses match the filters</div>
        ) : (
          <table className="w-full text-sm">
              ...
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    <Link to="/expenses/$id" params={{ id: r.id }} className="text-primary hover:underline">{r.category}</Link>
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums text-destructive">{formatCurrency(r.amount, currency)}</td>
                  <td className="px-4 py-3 text-muted-foreground">{accountsById[r.accountId] ?? "—"}</td>
            ...
            <tfoot>
              <tr className="border-t border-border bg-muted/20 text-sm font-semibold">
                <td colSpan={2} className="px-4 py-3">Total ({rows.length})</td>
                <td className="px-4 py-3 text-right tabular-nums text-destructive">{formatCurrency(total, currency)}</td>
                <td colSpan={2} />
              </tr>
            </tfoot>
          </table>
