...
import { Undo2, Search, Trash2 } from "lucide-react";
...
function PurchaseReturnsPage() {
  ...
  const { returns, allPurchases, hydrated, remove } = usePurchases(scopedBusinessId);
  ...
  const retPg = useListPagination(sorted, "purchase-returns");
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setDeleting(null);
      toast.success(`Deleted return ${n}`);
    } catch (err) {
      const message = err instanceof Error ? err.message : "Could not delete purchase return";
      toast.error(message);
    }
  };
      ...
      <main className="max-w-screen-2xl px-6 py-8">
        {hydrated && sorted.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            ...
            <ul className="divide-y divide-border">
              {retPg.pageItems.map((r) => (
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground">
                    {returnPaymentTypeLabel(r.returnPaymentMode)}
                  </span>
            ...
            <ListPaginationBar
              page={retPg.page}
              totalPages={retPg.totalPages}
              totalCount={retPg.totalCount}
              rangeFrom={retPg.rangeFrom}
              rangeTo={retPg.rangeTo}
              onPageChange={retPg.setPage}
            />
          </div>
        )}
      </main>
      ...
      <AlertDialog open={!!deleting} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the purchase return and any linked refund receipt from cash/bank. Net
              profit and dashboard totals will update.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
