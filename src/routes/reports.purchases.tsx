...
function PurchaseReport() {
  ...
  const exportCsv = () => {
    downloadCsv(
      ...
      rows.map((r) => [r.number, format(new Date(r.date), "yyyy-MM-dd"), r.partyName, r.total.toFixed(2), r.status]),
    );
  };
  ...
  return (
          ...
          <div className="min-w-[200px]">
            ...
            <Select value={supplierId} onValueChange={setSupplierId}>
              ...
              <SelectContent>
                ...
                {suppliers.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          {(from || to || supplierId !== "all") && (
            <Button variant="ghost" size="sm" onClick={() => { setFrom(""); setTo(""); setSupplierId("all"); }}>
              Reset
            </Button>
          )}
        </>
    ...
    >
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        {rows.length === 0 ? (
          <div className="px-6 py-16 text-center text-sm text-muted-foreground">No purchases match the filters</div>
        ) : (
          <table className="w-full text-sm">
              ...
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-muted/30">
                  <td className="px-4 py-3 font-mono text-xs">
                    <Link to="/purchases/$id" params={{ id: r.id }} className="text-primary hover:underline">{r.number}</Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className={cn(
                      ...
                      r.status === "final" ? "bg-success/15 text-success" : "bg-muted text-muted-foreground",
                    )}>{r.status}</span>
                  </td>
                </tr>
              ))}
            ...
            <tfoot>
              <tr className="border-t border-border bg-muted/20 text-sm font-semibold">
                ...
                <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(total, currency)}</td>
                <td />
              </tr>
            </tfoot>
          </table>
